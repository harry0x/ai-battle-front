...
export default function ChatWindow({ chatId }) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useMessages(isGuest ? null : chatId);
    ...
    try {
      ...
      const onEvent = (event) => {
        if (event.type === "user_message") {
          ...
          if (event.type === "mistral_chunk")
            currentData.mistral += event.chunk;
          if (event.type === "cohere_chunk") currentData.cohere += event.chunk;

          setStreamingMessage((prev) =>
            prev ? { ...prev, content: JSON.stringify(currentData) } : prev,
          );
          ...
          currentData.judge.mistralReasoning =
            currentData.judge.solution_1_reasoning;
          currentData.judge.cohereReasoning =
            currentData.judge.solution_2_reasoning;
  ...
  const isGenerating =
    !!(tempUserMessage || streamingMessage) ||
    (sendMessage && sendMessage.isPending);
      ...
      {isGuest && (
        <div className="glass border-b border-border px-6 py-3 flex items-center justify-between z-10 animate-fade-up">
          <div className="flex items-baseline select-none">
            <span className="text-2xl font-medium tracking-wider text-text-secondary font-mono lowercase">
              versus
            </span>
            <span className="text-4xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-accent to-orange-400 ml-0.5">
              AI
            </span>
          </div>
      ...
      {isEmpty ? (
        <div className="flex-1 flex flex-col items-center justify-center p-4">
          ...
          <div className="text-center mb-10 animate-fade-up">
            <div className="w-14 h-14 rounded-2xl overflow-hidden mx-auto mb-5 animate-pulse-glow shadow-lg shadow-accent/20">
              <img
                src="/versusai-logo.png"
                alt="VersusAI"
            ...
            <p className="text-sm text-text-muted max-w-md leading-relaxed">
              Two AI agents compete to give you the best answer.
              <br />A judge agent evaluates and picks the winner.
            </p>
          </div>

          {/* Quick suggestions */}
          <div
            className="flex flex-wrap justify-center gap-2 mb-8 max-w-2xl animate-fade-up"
